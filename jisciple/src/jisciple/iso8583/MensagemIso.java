...
public class MensagemIso {
	...
	public byte[] toByteArray() {
		...
		if (sizeHeader.getIncluiProprioTamanho()) {
			// System.out.println("adicionando tam " + sizeHeader.getTamanho() + " ao header
			// de tamanho " + length);
		...
		if (sizeHeader.getTipo().equalsIgnoreCase("ASCII")) {
			...
			if (sizeHeader.getFormato().equalsIgnoreCase("LH")) {
				headerStr = headerStr.substring(sizeHeader.getTamanho() / 2)
						+ headerStr.substring(0, sizeHeader.getTamanho() / 2);
			}
		...
		} // essa gambi de pegar metade do tamanho do header so funciona pra 2 ou 4, mas
			// good enough
	...
	public String prettyPrint() {
		...
		for (int i = 1; i < 128; i++) {
			if (dados[i] != null && dados[i].getValor() != null) {
				sb.append(String.format("%6d (%3d): [%s]\n", (i + 1), dados[i].getValor().length,
						getReadableString(dados[i])));
			}
		}
	...
	public void check() {
		...
		if (Arrays.compare(bitmap1calc.getValor(), bitmap1.getValor()) != 0) {
			...
			System.err.println("Bitmap 1 diferente: calculado [" + getReadableString(bitmap1calc) + "], atual ["
					+ getReadableString(bitmap1) + "]");
		}
		...
		if (bitmap1.useBitmap2()) {
			...
			if (Arrays.compare(bitmap2calc.getValor(), dados[0].getValor()) != 0) {
				System.err.println("Bitmap 2 diferente: calculado [" + getReadableString(bitmap2calc) + "], atual ["
						+ getReadableString(dados[0]) + "]");
			}
		}
